# Linux-Essential
Welcome to the Linux Command Reference guide. This document is designed to provide you with quick access to a wide range of Linux commands and system management topics. Whether you're a seasoned Linux user or just starting out, you'll find this guide useful for everyday tasks and advanced troubleshooting.
## Table of Contents
...
8. [System Monitoring and Troubleshooting](#system-monitoring-and-troubleshooting)
9. [File Compression and Archiving](#file-compression-and-archiving)
10. [Scripting and Automation](#scripting-and-automation)
11. [Editor Commands](#editor-commands)
...
- [Differences of `/etc/profile`, `~/.bash_profile`, `~/.bashrc`](#differences-of-etcprofile-~bash_profile-~bashrc)
- [Switch to root environment (Ubuntu)](#switch-to-root-environment-ubuntu)
- [Change password](#change-password)
- [Restart bash](#restart-bash)
- [List all commands recognized by Linux shell](#list-all-commands-recognized-by-linux-shell)
- [Check manual of a command](#check-manual-of-a-command)
- [Find out where `<other-command>` exists](#find-out-where-other-command-exists)
- [Show all environment variables](#show-all-environment-variables)
- [Find an environment variable by name](#find-an-environment-variable-by-name)
- [Export an environment variable](#export-an-environment-variable)
- [Concat string to an environment variable](#concat-string-to-an-environment-variable)
- [Append current directory to the environment variable `$PATH`](#append-current-directory-to-the-environment-variable-path)
- [Remove an environment variable `<ENV_VAR_NAME>`](#remove-an-environment-variable-env_var_name)

## System Information

- [Check system information (either of one)](#check-system-information-either-of-one)
- [Check Ubuntu system information (version)](#check-ubuntu-system-information-version)
- [Check Ubuntu architecture](#check-ubuntu-architecture)
- [Check CentOS/RHEL OS update level (version)](#check-centosrhel-os-update-level-version)
- [Check CUDA version](#check-cuda-version)
- [Check cuDNN version](#check-cudnn-version)
- [Check if your machine is physical or virtual (either of one)](#check-if-your-machine-is-physical-or-virtual-either-of-one)
- [Check CPU specification](#check-cpu-specification)
- [Check CPU microarchitecture](#check-cpu-microarchitecture)
- [Get logical core count](#get-logical-core-count)
- [Check external devices' information](#check-external-devices-information)
- [Check GPU driver version](#check-gpu-driver-version)
- [Monitor GPU usage](#monitor-gpu-usage)
- [Check GPU specification](#check-gpu-specification)
- [Enhanced `nvidia-smi`, showing processes' info](#enhanced-nvidia-smi-showing-processes-info)

## File and Directory Management

- [Check file structure](#check-file-structure)
- [Find out the device where a directory is on](#find-out-the-device-where-a-directory-is-on)
- [Create a directory `<dir-name>`](#create-a-directory-dir-name)
- [Rename a batch of files](#rename-a-batch-of-files)
- [Combine files by column, and set the column width](#combine-files-by-column-and-set-the-column-width)
- [Shuffle a file](#shuffle-a-file)
- [Random sample from a file](#random-sample-from-a-file)
- [Create a `.bak` file for `<filename>`](#create-a-bak-file-for-filename)

## Package Management

- [Show package description](#show-package-description)
- [Show package version details](#show-package-version-details)
- [Set (or unset) apt proxy](#set-or-unset-apt-proxy)
- [Set (or unset) system proxy](#set-or-unset-system-proxy)
- [Install a package](#install-a-package)
- [Uninstall a package](#uninstall-a-package)
- [Upgrade a package](#upgrade-a-package)
- [Download `.deb` and not install](#download-deb-and-not-install)
- [Install a `.deb`](#install-a-deb)
- [List packages installed](#list-packages-installed)
- [List installed packages by apt](#list-installed-packages-by-apt)
- [Check the installation path of a package](#check-the-installation-path-of-a-package)
- [List all versions of a package (CentOS)](#list-all-versions-of-a-package-centos)
- [Search packages (CentOS)](#search-packages-centos)

## Networking

- [Get host name](#get-host-name)
- [Get IP address](#get-ip-address)
- [Restart a specific network interface](#restart-a-specific-network-interface)
- [Check unused IP address](#check-unused-ip-address)
- [Configure static (fixed) IP address](#configure-static-fixed-ip-address)
- [Configure DNS](#configure-dns)
- [Check port usage (either of one)](#check-port-usage-either-of-one)
- [Check the status of a machine's specific port](#check-the-status-of-a-machines-specific-port)
- [Check the process using the port `<port-number>`](#check-the-process-using-the-port-port-number)
- [Setup SSH server](#setup-ssh-server)
- [Connect to remote host](#connect-to-remote-host)
- [Connect to remote host's `<port-number>`](#connect-to-remote-hosts-port-number)
- [Send command to remote host and get result locally](#send-command-to-remote-host-and-get-result-locally)
- [Keep SSH connection alive](#keep-ssh-connection-alive)
- [Remove host name from known_hosts file](#remove-host-name-from-known_hosts-file)
- [SSH with password](#ssh-with-password)
- [Check SSH server status](#check-ssh-server-status)
- [Allow SSH server starts on reboot](#allow-ssh-server-starts-on-reboot)
- [Check whether a service is enabled](#check-whether-a-service-is-enabled)

## User and Permission Management

- [Manage user and group](#manage-user-and-group)
- [Manage privilege of a file](#manage-privilege-of-a-file)
- [List all users](#list-all-users)

## Process Management

- [Find process ID and then kill it](#find-process-id-and-then-kill-it)
- [Kill all processes using GPU](#kill-all-processes-using-gpu)
- [Measure running time of a command](#measure-running-time-of-a-command)

## System Monitoring and Troubleshooting

- [Check boot/shutdown log](#check-bootshutdown-log)
- [Solve `NVIDIA NVML Driver/library version mismatch`](#solve-nvidia-nvml-driverlibrary-version-mismatch)
- [Check system log around the date `<yyyymmdd>`](#check-system-log-around-the-date-yyyymmdd)
- [Check Ubuntu cannot boot: `platform MSFT0101:00 failed to claim resource 1`, `acpi MSFT0101:00: platform device creation failed: -16`](#check-ubuntu-cannot-boot-platform-msft010100-failed-to-claim-resource-1-acpi-msft010100-platform-device-creation-failed-16)
- [Check Ubuntu cannot boot: black screen with blinking underscore](#check-ubuntu-cannot-boot-black-screen-with-blinking-underscore)

## File Compression and Archiving

- [Zip `<foldername>` to `<foldername>.zip`](#zip-foldername-to-foldernamezip)
- [Unzip troubleshooting](#unzip-troubleshooting)
- [Unzip a file to a designated directory](#unzip-a-file-to-a-designated-directory)
- [Pack to a tar file](#pack-to-a-tar-file)
- [Compress to a gz file](#compress-to-a-gz-file)
- [Compress to a tar.gz file](#compress-to-a-targz-file)
- [Compress to a tar.gz file, excluding some subfolders](#compress-to-a-targz-file-excluding-some-subfolders)
- [Show progress bar while compressing to a tar.gz file](#show-progress-bar-while-compressing-to-a-targz-file)
- [Show progress bar while unzipping a tar.gz file](#show-progress-bar-while-unzipping-a-targz-file)
- [Extract .tar file](#extract-tar-file)
- [Extract .tar.gz file](#extract-targz-file)
- [Extract .tar.gz file to a directory](#extract-targz-file-to-a-directory)
- [Extract .gz file](#extract-gz-file)
- [Extract .7z file](#extract-7z-file)
- [Extract .bz2 file](#extract-bz2-file)
- [Extract .rar file](#extract-rar-file)

## Scripting and Automation

- [For loop in bash](#for-loop-in-bash)
- [Pass argument to shell script](#pass-argument-to-shell-script)
- [Make the output of a command be a command and then execute it](#make-the-output-of-a-command-be-a-command-and-then-execute-it)

## Editor Commands

- [Nano editor](#nano-editor)
- [Emacs editor](#emacs-editor)
- [Open folder from terminal](#open-folder-from-terminal)
- [Open file from terminal](#open-file-from-terminal)

## Miscellaneous

- [Get video codec](#get-video-codec)
- [Get video information](#get-video-information)
- [Video conversion: mp4 to h264](#video-conversion-mp4-to-h264)
- [Run cron job](#run-cron-job)
- [Create a symbolic link (soft link) of `<target>` named `<linkname>`](#create-a-symbolic-link-soft-link-of-target-named-linkname)
- [Check the target of a symbolic link](#check-the-target-of-a-symbolic-link)
- [Remove a symbolic link](#remove-a-symbolic-link)
- [Mount /src/dir to /dst/dir](#mount-srcdir-to-dstdir)
- [Unmount /dst/dir](#unmount-dstdir)
- [NTFS partition is read only, to make it writable](#ntfs-partition-is-read-only-to-make-it-writable)
- [Check the contents of a JAR file](#check-the-contents-of-a-jar-file)
- [Check the contents of a ZIP file](#check-the-contents-of-a-zip-file)
- [Perform set operation on `<file1>` and `<file2>`](#perform-set-operation-on-file1-and-file2)
- [Ab: Test the capability of serving of a website](#ab-test-the-capability-of-serving-of-a-website)
- [To exit X server](#to-exit-x-server)

## Advanced Topics

- [List the last `<num_lines>` lines in `<filename.txt>`](#list-the-last-num_lines-lines-in-filename.txt)
- [Display the output of `<other-command>` in pages and scroll to bottom automatically](#display-the-output-of-other-command-in-pages-and-scroll-to-bottom-automatically)
- [Run your command in background, write its output to a text file, and watch that text file simultaneously](#run-your-command-in-background-write-its-output-to-a-text-file-and-watch-that-text-file-simultaneously)
- [Alias: List alias set previously](#alias-list-alias-set-previously)
- [Alias: Create an alias of a command](#alias-create-an-alias-of-a-command)
- [Compare the two files, show the different byte position and their values](#compare-the-two-files-show-the-different-byte-position-and-their-values)
- [Compare the two so or executable files](#compare-the-two-so-or-executable-files)
- [Create a `.bak` file for `<filename>`](#create-a-bak-file-for-filename)
- [Find unique elements from other command's output](#find-unique-elements-from-other-commands-output)
- [Translate or delete characters - squeeze multiple `<char>` into one](#translate-or-delete-characters-squeeze-multiple-char-into-one)
- [Delete characters](#delete-characters)
- [Replace characters](#replace-characters)
- [Delete `'\r'` in files](#delete-r-in-files)
- [Cut by `<delimiter>` and get the specific `<fields>`](#cut-by-delimiter-and-get-the-specific-fields)
- [Cut a file by `<delimiter>` and get the specific `<fields>`](#cut-a-file-by-delimiter-and-get-the-specific-fields)
- [Cut by space and get the last element](#cut-by-space-and-get-the-last-element)
- [Cut by space and remove the last element](#cut-by-space-and-remove-the-last-element)
- [Join by a delimiter](#join-by-a-delimiter)
- [Reverse the output of `<other-command>`](#reverse-the-output-of-other-command)
- [Reverse a file](#reverse-a-file)
- [Use curl to test REST API](#use-curl-to-test-rest-api)
- [Download a file as `<directory/file_name>` from a URL](#download-a-file-as-directoryfilename-from-a-url)
- [Curl, ignore SSL certificate warnings](#curl-ignore-ssl-certificate-warnings)
- [Download a file to `<dirname>` from a URL](#download-a-file-to-dirname-from-a-url)
- [Download a file only when the file does not exist](#download-a-file-only-when-the-file-does-not-exist)
- [Iterate over the result of some command](#iterate-over-the-result-of-some-command)
- [Let the command 'history' show the time when the commands are executed](#let-the-command-history-show-the-time-when-the-commands-are-executed)
- [Output non-ASCII characters in shell](#output-non-ascii-characters-in-shell)
- [Write or append the output to a file](#write-or-append-the-output-to-a-file)
- [Redirect the error message of a command to a file](#redirect-the-error-message-of-a-command-to-a-file)
- [Redirect to both file and console (stdout)](#redirect-to-both-file-and-console-stdout)

---

Feel free to expand each section with detailed instructions and examples as needed.
